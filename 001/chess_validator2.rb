...
class Board
  BLANK = "--"
	...
	def is_bishop?(position)
		piece_at(position).include?("B")
	end

	def is_rook?(position)
		piece_at(position).include?("R")
	end

	def is_king?(position)
		piece_at(position).include?("K")
	end

	def is_queen?(position)
		piece_at(position).include?("Q")
	end	

	def is_knight?(position)
		piece_at(position).include?("N")
	end
	...
	def valid_white_pawn_move(starting_position, ending_position)
		#true if row at ending position is 1 less than starting position
		starting_position_row = starting_position[1].to_i 
		ending_position_row 	= ending_position[1].to_i
